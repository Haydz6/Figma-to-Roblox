        ...
        switch (Filler.type) {
            case "SOLID":
                ...
                if (Properties.Class == "TextLabel") {
                    ...
                    Properties.TextTransparency = Filler.visible ? Filler.opacity : 0
                } else {
                    Properties.BackgroundColor3 = Colour;
                    Properties.BackgroundTransparency = Filler.visible ? Filler.opacity : 0
                }
...
function CreateRobloxElement(Properties, ZIndex) { // Creates the roblox xml for the element
    var XML = "";
    ...
    ExtendXML(`<int name="ZIndex">${ZIndex}</int>`)
...
function ConvertToRoblox(Objects) { // Converts the code into roblox xml format
    ...
    for (var i = 0; i < Objects.length; i++) {
        XML += CreateRobloxElement(Objects[i], i);
    }
